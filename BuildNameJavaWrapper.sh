...
Config_FileName4="mdNameJavaWrapper.cpp"
...
########################## Main ############################
printf "\n======================= WELCOME TO MELISSA NAME OBJECT JAVA WRAPPER =======================\n"
...
# Compile SO
gcc -fpic -c -I$ProjectPath -I$JavaIncludeDirectory1 -I$JavaIncludeDirectory2 mdNameJavaWrapper.cpp
gcc -g -shared mdNameJavaWrapper.o -o libmdNameJavaWrapper.so -L$ProjectPath -lmdName -pthread

rm mdNameJavaWrapper.o

# Compile Jar
javac com/melissadata/*.java &> /dev/null

jar cvf mdName.jar com/melissadata/*.class &> /dev/null

if [ -f "libmdNameJavaWrapper.so" ];
then
  script_dir="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
  file_path="$script_dir/libmdNameJavaWrapper.so"

  printf "\nlibmdNameJavaWrapper.so has been successfully generated at:\n$file_path\n"
else
  printf "\nError: was not able to create libmdNameJavaWrapper.so\n"
fi

if [ -f "mdName.jar" ];
then
  script_dir="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
  file_path="$script_dir/mdName.jar"

  printf "\nmdName.jar has been successfully generated at:\n$file_path\n"
else
  printf "\nError: was not able to create mdName.jar\n"
fi
